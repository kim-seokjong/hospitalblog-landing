...
7. ★배경 생활감(2026-08-17 신설): 실제로 쓰이고 있는 공간처럼 채운다. 텅 빈 방은 모델하우스로 읽히고,
   ...
   ⇒ **크기가 아니라 「무엇을 적느냐」로 가른다**(9/7 대표 지적으로 고침):
     ⛔**글자를 크고 또렷하게 만들지 마라.** 처음엔 「크게 잡으면 오타가 없다」로 갔는데
     ...
     ✅**정자체를 요구하지 말고 「인쇄 표 + 손글씨」를 섞어라.** 그래야 실제 차트처럼 읽힌다.
       ...
       ⛔짧은 필드명에 정자체를 요구하면 오타가 난다(실측: 「성영」·「생년밀일」·「연악처」).
     ⛔여전히 빼야 할 것: 라벨 붙은 약병, 포스트잇, 명찰, 켜져 있는 모니터, 벽 안내문.
       모니터는 꺼진 화면으로 지정한다.
     ★★**소품의 방향은 「카메라」가 아니라 「그 장면 안에서 그것을 읽는 사람」이 정한다**
       (9/7 대표 지적). 의사가 환자에게 설명하는 중이면 서류는 **환자 쪽으로 돌아가 있어야**
       하고, 그러면 카메라에서는 글자가 옆으로 눕는다. 화면에 잘 보이게 놓으면 연출 사진이 된다.
       ⇒ "the document is turned toward the person who is reading it, not toward the camera,
          so its text runs sideways from this angle" 처럼 **읽는 사람을 기준으로** 쓴다.
