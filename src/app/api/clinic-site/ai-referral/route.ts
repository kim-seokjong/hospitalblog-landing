 ...
 *  3. HMAC 서명 토큰 대조 (발급: GET /api/clinic-site/ai-referral/token).
 *     보증하는 것은 "우리 서버가 10분 안에 발급했고 (slug·source·postId) 가 서명
 *     시점과 같다" 까지다 — 사람인지·정말 AI 에서 왔는지·처음 쓰는 토큰인지는
 *     보증하지 않는다(발급 경로가 공개라 누구나 유효 토큰을 얻는다).
 *     즉 위조 **비용**을 올리는 장치이며, 서명이 아예 없으면 단 한 번의 POST 로
 *     누구나 임의 병원 통계를 조작할 수 있다는 점에서 여전히 필수다.
 *     시크릿 미설정이면 전부 거부(기능 비활성).
 *  4. 레이트리밋 — 실제로 피해 규모를 묶는 장치. 키는 해시된 IP 이며
