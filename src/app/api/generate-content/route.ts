...
- 소제목 첫 문장을 "정의문"으로 시작하지 말 것 — 학술 논문처럼 보임
  대신: 환자의 일상 상황·관찰·질문으로 자연스럽게 도입
  예시(좋음): "허리 한쪽이 아침마다 묵직하다는 분들이 의외로 많다."
  예시(나쁨): "추간판 탈출증은 디스크 수핵이 섬유륜을 뚫고 나오는 질환이다."
- 통계·수치·퍼센트 나열을 자제 — 꼭 필요한 1~2개만 자연스럽게 녹일 것
- "○○학회 가이드라인", "임상 연구 결과" 같은 인용형 표현 사용 금지
- AI가 쓴 듯한 단정형·요약형 문장 패턴(자족 문장) 줄이고, 사람의 흐름 있는 서술로 작성
- 같은 단어·표현을 너무 자주 반복하지 말고, 동의어와 풀어쓰기를 섞을 것`;

    const baseStyleBlock = isGeoMode ? `
【자연스러운 글쓰기 원칙】
...
【자연스러운 글쓰기 원칙 — 강화】
- 소제목은 검색자가 실제 궁금해할 법한 말투로 (질문형, 일상어 권장)
- 주요 소제목 앞뒤 빈 줄로만 구분, 기호 없음
- 문장마다 시작 방식을 다양하게 — 같은 패턴 반복 금지
- 짧은 문장과 긴 문장을 자연스럽게 섞고, 가끔 의도적인 도치/생략으로 리듬감 부여
- 한 단락 안에서도 호흡이 길어지면 나누고, 짧으면 합치는 식으로 유연하게
- "환자분들 중에는", "임상에서 자주 보는", "실제로는" 같은 사람이 자연스럽게 쓰는 도입어 활용
- 모든 단락을 정의·결론·통계로만 구성하지 말 것 — 사례, 관찰, 비교, 질문 등 다양한 결로 풀 것
- 단락은 2~4문장이지만, 흐름상 자연스러우면 1문장 또는 5문장도 허용`;
...
- 장면을 구체적으로: 시술 단계(준비/시술중/직후/회복기), 사용 도구, 시술 부위, 의료진 동작, 환자 표정·자세 중 최소 2가지 명시
- 시각 단서를 구체적으로: 어떤 도구가 보이는지(예: 미세주사기, 메소건, 레이저 핸드피스), 시술 부위는 어디인지(예: 미간, 광대, 무릎 슬개골), before/after 구분이 있는지
- 5~6장 모두 다른 각도·단계: 클로즈업 / 미디엄샷 / 시술 도구 단독 / 의료진과 환자 상호작용 / 시술 전후 비교 split / 회복 직후 등으로 분산 (같은 구도 반복 금지)
- 한 줄당 30~50자, 길지 않게

좋은 예시 (키워드: 보톡스):
[이미지 1: 환자 미간에 마킹된 주사 포인트와 의료진의 미세 주사기 클로즈업]
[이미지 2: 보톡스 시술 전후 미간 주름 비교 split frame]
[이미지 3: 시술 직후 환자 측면, 시술 부위 거울로 확인하는 모습]

좋은 예시 (키워드: 물광):
[이미지 1: 메소건이 광대 부위에 닿기 직전 클로즈업, 작은 주사 자국]
[이미지 2: 시술 후 광택이 도는 촉촉한 피부 결 클로즈업]
[이미지 3: 시술 전 건조한 피부 vs 시술 후 글로우 split frame]

나쁜 예시 (피해야 함):
[이미지 1: 보톡스 관련 의료 장면] — 추상적
[이미지 2: 병원에서 상담 받는 환자] — 키워드 특이성 없음
[이미지 3: 의료진 모습] — 시술 식별 불가${summaryAndFaqSection}
