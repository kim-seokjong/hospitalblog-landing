...
/**
 * GET /api/cron/registry-health — 행안부 병원 조회 카나리 점검 (하루 1회, KST 08:00).
 *
 ...
 *
 * ⚠️⚠️ 스케줄을 매시간(`0 * * * *`)으로 되돌리지 마라 — **배포가 통째로 죽는다**.
 *   이 프로젝트의 Vercel 계정은 **Hobby** 플랜이고, Hobby 는 cron 을 하루 1회만
 *   허용한다. 하루 2회 이상이 되는 표현은 실행 시점이 아니라 **배포 단계에서**
 *   실패한다("Hobby accounts are limited to daily cron jobs").
 *   실제로 2026-07-27 19:11 에 `0 * * * *` 로 올렸다가 그때부터 2026-07-28 까지
 *   **9개 커밋이 전부 배포 실패**했다 — 병원 조회 폴백·메일 실패 알림·랜딩 수정이
 *   전부 프로덕션에 못 올라간 채 쌓였다. 이 사고를 막으려고 만든 감시장치가
 *   정작 그 사고의 수정본 배포를 막았다.
 *   → 시간당 감시가 필요하면 (a) Vercel Pro 로 올리거나 (b) 이미 쓰고 있는
 *     Railway 스케줄러에서 이 엔드포인트를 호출한다. vercel.json 을 고치지 마라.
 *   → 회귀 방지: `src/dev/lib/__tests__/cron-frequency.test.ts` 가 vercel.json 의
 *     모든 cron 을 검사해 하루 2회 이상이면 테스트가 깨진다.
 *
 * 하는 일:
 ...
 *
 * 비용: 카나리 2개 × 보통 1콜 = 하루 2콜. 일일 한도(10,000) 대비 무시할 수준.
 *
