...
/**
 * 폴백 명부(심평원 공개자료)로 찾은 결과의 캐시 상한 — 1시간.
 *
 * 폴백은 **행안부가 죽어 있는 동안의 대타**다. 행안부가 20분 만에 복구돼도 폴백
 * 결과를 하루 동안 물고 있으면, 그 하루는 정본이 아닌 자료로 답하게 된다
 * (폴백에는 폐업 구분이 없고 최근 개원이 빠져 있다).
 * 1시간이면 장애 중 반복 조회는 흡수하면서 복구 후에는 곧 정본으로 돌아온다.
 */
export const LOOKUP_DIRECTORY_CACHE_TTL_MS = 60 * 60 * 1000;
