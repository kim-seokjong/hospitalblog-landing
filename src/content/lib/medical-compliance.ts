...
/**
 * 광고 CTA 조각 — 승격 규칙들이 **공유**한다. 한 곳에서만 고친다.
 *
 * ⚠️어간 조각(`받`·`가능`·`제공`)을 쓰면 안 된다. 2026-08-13 교차검증에서
 *   "치료 정보를 **받**아들이기", "치료 **가능**성", "**제공**되는 정보" 가 전부
 *   CRITICAL 로 올라갔다. **활용 종결까지 포함한 형태**만 넣는다.
 * ⚠️`안심하고`·`보장` 단독도 금지 — "안심하고 방치해서는 안 됩니다"(경고문),
 *   "사회보장 제도"(무관)를 문다. 뒤따르는 행위와 결합해야 CTA 다.
 */
/**
 * "보장" 뒤의 **단순 부정만** 면제하는 전방탐색 조각.
 *
 * "효과를 보장하지 않는다"(의료광고법이 요구하는 고지문)는 면제하되,
 * ⚠️**이중부정은 다시 잡는다** — "보장하지 않는 것은 아닙니다", "보장하지 않을 수 없습니다",
 *   "보장되지 않을 리 없습니다" 는 의미상 결과 보장 광고다(2026-08-13 4라운드 지적).
 */
// ⚠️`아니` 로 끝내면 "아**닙**니다"를 놓친다 — 두 번째 음절이 `니`가 아니라 `닙`이다.
//   `받으시`→"받으실" 과 같은 음절 누락 계열이라 `아[니닙님]` 로 받는다.
//   조사도 `것은`·`것이` 둘 다 온다.
const DOUBLE_NEG =
  '(?:는\\s*(?:것|게|건|바)[은는이가]?\\s*아[니닙님]|' +
  '는다고\\s*(?:볼|할)\\s*수(?:는|가)?\\s*없|' +
  '을\\s*수(?:는|가|밖에)?\\s*없|을\\s*리\\s*없|는\\s*법이\\s*없)';
/**
 * 읽는 법: "보장" 뒤가 **단순 부정이면 매칭하지 않는다.** 단 그 부정이 다시 이중부정으로
 * 이어지면(안쪽 `(?!…DOUBLE_NEG)` 가 실패) 바깥 배제가 풀려 **다시 잡힌다.**
 */
const NEG_NOT_DOUBLE =
  '(?!성)' +
  `(?!(?:하지|되지)\\s*(?:는|도)?\\s*않(?!\\s*${DOUBLE_NEG}))` +
  '(?!할\\s*수\\s*없)(?!하기\\s*어렵)';

/**
 * ⚠️`제공합니다`·`해 드립니다` 는 **넣지 않는다.** 정상 안내문을 문다 —
 *   "관련 정보를 제공합니다", "…있음을 안내해 드립니다"(2026-08-13 3라운드 지적).
 *   시술 명사에 **직접 붙은** 형태만 아래 승격②에서 따로 받는다.
 * ⚠️`보장`·`안심하고` 는 **부정 극성**을 배제해야 한다 —
 *   "효과를 보장하지 않는다", "안심하고 선택해서는 안 됩니다" 는 정상 문장이다.
 */
const AD_CTA =
  // ⚠️문자 클래스보다 의도가 드러나는 나열로 둔다. `받으셔`·`받으셨` 계열이 빠져 있었다.
  '받으(?:세|시|실|신|셔|셨|러)|받아\\s*보|받을\\s*수|받게\\s*되|받도록|' +
  '시작하세요|경험하세요|경험해\\s*보|만나\\s*보세요|누려\\s*보|누리세요|' +
  '안심하고\\s*[^.!?\\n]{0,6}(?:선택|내원|방문|시술|치료|결정|상담)\\s*' +
  '(?:하세요|하실\\s*수|받으|해\\s*보|가능)|' +
  `(?:효과|결과|만족)[을를]?\\s*보장${NEG_NOT_DOUBLE}|책임집니다`;

/**
 * 위해 결과 명사 — 무위해 단정 계열 규칙이 **전부** 공유한다.
 * ⛔**`멍` 은 넣지 않는다(2026-08-13 결정).** 한 글자라 경계를 아무리 둘러도 합성어가
 *   계속 새로 나온다 — 구멍·멍울(의학 용어)·멍청·멍게·멍석·멍에·멍때림… 좌측 경계와
 *   `(?!울|청)` 까지 붙였는데도 교차검증이 매 라운드 새 후보를 찾아냈다.
 *   "멍 없이 시술받으세요" 를 놓치는 손해보다, 정상 글이 CRITICAL 로 막히는 손해가 크다.
 *   **한 글자 위해어는 A층 어휘 규칙의 사정거리 밖**이고 B층 LLM 심의의 몫이다.
 *   (2026-06-15 `기적`→'정기적인' 사고와 같은 계열)
 */
const HARM_NOUNS = '부작용|후유증|합병증|통증|흉터|붓기|염증|감염|재발|출혈';

/**
 * 조건·양보 어미 — `않-` 뒤에 붙어 **단정이 아님을 드러내는** 형태.
 * 세 규칙(CRITICAL 단정 배제 / MEDIUM 조건형 / CRITICAL 승격)이 공유해 **분기가 어긋나지 않게** 한다.
 * ⚠️`않을 경우`·`않는 경우`·`않아도`·`않더라도` 는 실제 임상 글에 매우 흔하다 —
 *   "통증이 조절되지 않는 경우 수술을 고려합니다"(2026-08-13 5라운드 지적).
 */
const COND_ENDING = '(?:으면|을\\s*때|을\\s*경우|는\\s*경우|는다면|다면|아도|어도|더라도)';
...
const WARNING_PATTERNS: PatternRule[] = [
  ...
  // 2026-W34: 해시태그형 소비자 오인 표현. 인스타 병원 계정 실측(2026-07-15 하이업 점검)에서
  // 본문이 아니라 **해시태그**에 최상급이 들어가는 형태가 반복 확인됐다(#○○잘하는곳).
  // ★어휘 목록(FORBIDDEN_WORDS)이 아니라 패턴 경로에 둔다 — 그 목록은 autoFix() 치환을 겸해서
  //   '잘하는곳'을 치환하면 문장이 깨진다(2026-07-28 '타의 추종을 불허' 사고와 같은 계열).
  // ⚠️`1등` 은 `1등급`(정상 의학 표기)과 겹치므로 `(?!급)` 으로 잘라낸다.
  // ⚠️`유명한` 은 위 유명인 패턴과 겹치지 않게 '곳/병원' 결합형만 본다.
  {
    pattern: /잘하는\s*곳|잘하는곳|유명한\s*(?:곳|병원|의원)|1\s*등(?!급)|넘버\s*원|no\.?\s*1\b/i,
    message: '「잘하는 곳」·「1등」 같은 표현은 객관적 근거로 증명하기 어려워 소비자를 오인시킬 소지가 있습니다. 해시태그에 쓴 것도 광고 문구로 봅니다.',
    severity: 'MEDIUM',
    rule: '객관적 근거 없는 우월성·소비자 오인 표현 (의료법 제56조 제2항)',
    suggestion: '우월성 표현 삭제 — 진료 범위·경력 등 확인 가능한 사실로 대체',
  },
  // 2026-W34: 재생의료 용어. 복지부가 2025년에 재생의료 분야만 5개월 표적 모니터링해
  // 63개 기관 246건을 적발했다(전자책 부록 D 근거). 표적 단속이 붙은 영역이라 단독 등장만으로도
  // 표면화한다. ⛔차단은 하지 않는다(MEDIUM) — 실제로 시행하는 진료를 사실대로 적는 글이 많다.
  {
    pattern: /줄기\s*세포|엑소좀|exosome|자가혈\s*치료|prp\s*(?:주사|시술|치료)/i,
    message: '재생의료 관련 용어는 표적 모니터링 대상 분야입니다(2025년 복지부 재생의료 집중 점검). 효과를 단정하거나 시술을 권유하는 문맥이 아닌지 확인이 필요합니다.',
    severity: 'MEDIUM',
    rule: '재생의료 분야 광고 — 집중 점검 대상 (의료법 제56조)',
    suggestion: '효과 단정·권유 표현을 빼고 시행 사실과 대상 질환만 서술',
  },
  ...
  // 간격 8자를 허용해 "부작용 걱정 없이"·"부작용이 전혀 없습니다" 를 함께 잡는다.
  {
     ...
     */
    // ⚠️2026-08-13: 명사 목록을 HARM_NOUNS 로 통일했다. 출혈이 여기만 빠져 있어
    //   "출혈 없이 안전하게 수술받으세요" 가 탐지 0건이었다(3라운드 지적).
    //
    // ⛔알려진 공백(전 명사 공통, 출혈만의 문제가 아님): **부정문으로 감싼 무위해 문장**은
    //   그대로 CRITICAL 이다 — "부작용 없이 진행되는 시술이 아닙니다".
    //   여기에 예외를 만들면 "…없이 시술받으세요, 라고 말하는 것은 아닙니다" 류의
    //   회피구가 열린다. 부정 극성 판정은 문맥 층위라 B층 LLM 심의에 맡긴다.
    pattern: new RegExp(
      `(${HARM_NOUNS})[^.!?\\n]{0,8}(?<!(?:안내|설명|고지|동의|상담)\\s*)` +
        '(없이(?!도)|없는(?!지|\\s*(경우|환자|때|분|사람|케이스))|없다|없습니다|없어요|없음|없을(?!까))',
    ),
  ...
  /**
   * ═══ `없이도` — 배제가 아니라 **등급을 낮추고 승격으로 되올린다** ═══
   *
   * ⚠️ 2026-08-13 실측 오탐: *"통증 없이도 변형이 진행될 수 있어 초기 증상을 놓치기
   *    쉽습니다"* 가 CRITICAL 로 잡혀 발행 게이트가 걸렸다. **무위해 광고가 아니라
   *    무증상 진행을 경고하는 문장**이다. 무지외반증처럼 "통증 없이 진행된다"가
   *    핵심 임상 사실인 질환은 이 문장을 안 쓸 수가 없다.
   *
   * ⚠️⚠️ 처음엔 `없이(?!도)` 로 **완전 배제**했다가 교차검증에서 뒤집혔다.
   *    광고 쪽 허용목록(시술 명사 7종 + 권유 어미 10종)에 걸리지 않는 광고가 무더기로
   *    빠져나갔다 — "부작용 없이도 만족스러운 결과를 경험하세요", "통증 없이도
   *    임플란트를 받을 수 있습니다". 허용목록에서 **음절 하나만 빠져도 리콜 구멍**이
   *    생기는 구조였다(실제로 `받으시` 가 "받으실"을 못 잡고 있었다).
   *
   * ★ 그래서 **배제하지 않는다.** `없이도` 는 항상 검출하되 기본 MEDIUM(표면화만,
   *   발행 게이트 미발동)으로 두고, 광고 신호가 붙으면 CRITICAL 로 승격한다.
   *   ⇒ 정상 경고 문장은 발행을 막지 않고, 승격 허용목록에 구멍이 있어도 최악이
   *     MEDIUM 표면화라 **리콜이 0이 되지는 않는다.**
   *   같은 3단 구조를 "걱정 없이" 규칙이 이미 쓰고 있다(아래).
   *
   * ⚠️단 이 보장은 **HARM_NOUNS 목록과 8자 창 안에서만** 성립한다. 목록 밖 위해어나
   *   창을 넘는 수식어("…염려 하나 없이도")는 기본 패턴도 못 잡는다 — 이건 이 규칙
   *   고유의 한계가 아니라 무위해 단정 계열 전체의 기존 한계다(2026-08-13 교차검증 확인).
   */
  {
    // 기본 — 표면화만. 게이트(HIGH/CRITICAL)를 발동시키지 않는다.
    pattern: new RegExp(`(${HARM_NOUNS})[^.!?\\n]{0,8}없이도`),
    message:
      '“없이도”는 무증상 진행을 설명하는 정상 서술일 수 있지만, 광고 문맥에서는 무위해 단정으로 읽힙니다. 시술을 권하는 문장이면 삭제가 필요합니다.',
    severity: 'MEDIUM',
    rule: '허위 안전성·무위해 단정 주의 (의료법 제56조 제1항)',
    suggestion: '설명 문장이면 그대로 두어도 되지만, 권유 문장이면 “없이도” 삭제',
  },
  {
    /**
     * 승격 ① — 광고 CTA 가 붙은 경우. **시술 명사가 없어도** 잡는다
     *   ("부작용 없이도 만족스러운 결과를 경험하세요" — 교차검증 회피 사례).
     * ⚠️`하세요`·`가능합니다` 같은 범용 어미는 넣지 않는다 — "조기 발견이 가능합니다"
     *   같은 정상 서술을 CRITICAL 로 올려 원래 오탐이 재발한다. **권유·제공형만** 넣는다.
     */
    pattern: new RegExp(`(${HARM_NOUNS})[^.!?\\n]{0,8}없이도[^.!?\\n]{0,40}(${AD_CTA})`),
    message: '“없이도” 뒤에 시술 권유·제공 표현이 이어지는 문장은 무위해 단정 광고입니다(의료법 제56조 제1항).',
    severity: 'CRITICAL',
    rule: '허위 안전성·무위해 단정 금지 (의료법 제56조 제1항)',
    suggestion: '“없이도” 삭제 — 발생 가능성과 관리 방법 안내로 대체',
  },
  {
    /**
     * 승격 ② — 시술·의료행위 명사가 뒤따르는 경우.
     * ⚠️명사 목록은 계속 새는 축이라(레이저·임플란트·주사…) 넓게 잡되,
     *   뒤에 **활용 종결까지 갖춘** 행위 동사가 붙을 때만 승격한다. 명사만으로는 MEDIUM 에 머문다.
     * ⚠️`가능성`·`제공되는`·`받아들이` 를 물지 않도록 종결형으로 고정한다(교차검증 지적).
     */
    // ⚠️`해 드립니다`·`제공합니다` 는 AD_CTA 에서 뺐지만, 시술 명사에 **직접 붙은**
    //   형태("시술해 드립니다")는 명백한 광고라 여기서만 조사 1개까지 허용해 받는다.
    //   "치료를 안내해 드립니다" 처럼 사이에 다른 말이 끼면 안 잡히도록 창을 좁게 둔다.
    pattern: new RegExp(
      `(${HARM_NOUNS})[^.!?\\n]{0,8}없이도[^.!?\\n]{0,25}` +
        '(시술|치료|수술|성형|이식|교정|진료|검사|처방|투약|주사|약물|레이저|임플란트|보톡스|필러|리프팅|제모|마취|재활|케어|프로그램)' +
        `(?:[을를이가]?\\s*(?:해\\s*드립|해\\s*드려|가능합니다|가능해요|하실\\s*수\\s*있)|[^.!?\\n]{0,10}(?:${AD_CTA}))`,
    ),
    message: '“없이도” 뒤에 시술·의료행위 권유가 이어지는 문장은 무위해 단정 광고입니다(의료법 제56조 제1항).',
    severity: 'CRITICAL',
    rule: '허위 안전성·무위해 단정 금지 (의료법 제56조 제1항)',
    suggestion: '“없이도” 삭제 — 발생 가능성과 관리 방법 안내로 대체',
  },
   ...
   *     · 기본(아래 첫 규칙): 위해 명사 + 걱정 없이 → MEDIUM 표면화.
   *     · 승격(아래 둘째 규칙): 그 뒤에 **시술·치료·수술 권유**가 이어지면 CRITICAL.
   *       '상담·문의' 는 시술 명사가 아니라 자동으로 빠진다(따로 예외를 두지 않는다).
   *
   * ⚠️ **남는 한계(9라운드에서 확인, 의도적으로 여기서 멈춘다).**
   *   창을 60자로 넓혔지만 그보다 더 긴 수식어를 끼우면 여전히 빠져나간다:
   *     "부작용에 대해서는 충분한 설명과 상담 및 개인별 진료 계획을 자세하고
   *      친절하게 충분히 안내드리니 걱정 없이 시술받으세요"
   *   창을 계속 늘리면 결국 문장 전체 스캔이 되고, 그건 앞서 O(n²) 로 되돌아간
   *   길이다. 3~9라운드에서 확인된 것은 하나다 — **"환자를 안심시키는 안내"와
   *   "무위해 광고"의 경계는 어휘 규칙으로 그을 수 없다.** 그건 문맥 판단이고
   *   B층 LLM 심의의 일이다. A층은 여기까지가 정직한 한계다.
   *   ※ 이 한계를 없애려 창을 더 늘리거나 예외를 추가하지 마라. 3~9라운드가
   *     그 시도의 기록이고, 매번 회피구나 오탐 중 하나가 생겼다.
   */
  {
    pattern: /(부작용|후유증|합병증|통증|흉터|재발)[^.!?\n]{0,60}(걱정|우려)\s*(도|은|는)?\s*없이/,
    message:
  ...
  /**
   * 승격 — "걱정 없이" 뒤에 시술·치료 **권유**가 붙으면 명백한 무위해 광고다.
   * 예외가 아니라 **추가 규칙**이라 제외어를 끼워 넣는 회피가 성립하지 않는다.
   *
   * ⚠️ 9라운드: 처음엔 시술·치료 **명사만** 보고 승격했는데, 명사의 등장과 권유는
   *    다르다. "걱정 없이 상담받으시고 **치료 여부를 결정하세요**",
   *    "치료의 장단점을 확인하세요", "치료하지 않을 수도 있습니다" 가 전부
   *    CRITICAL 로 막혔다 — 오히려 권장되는 안내다.
   *    → 명사 뒤에 **권유 어미**(받으세요·하세요·시작하세요 …)가 붙을 때만 승격한다.
   *      '여부를 결정하세요'·'장단점을 확인하세요' 는 권유 어미가 아니라 빠진다.
   */
  {
    pattern:
      // ⚠️2026-08-13: `받으시` 는 "받으실"을 못 잡는다 — 마지막 음절이 `시`가 아니라 `실`이다.
      //   "걱정 없이 치료받으실 수 있습니다" 가 통째로 빠져나가던 리콜 구멍이었다. `받으[시실신]` 으로 넓힌다.
      /(부작용|후유증|합병증|통증|흉터|재발)[^.!?\n]{0,60}(걱정|우려)\s*(도|은|는)?\s*없이[^.!?\n]{0,25}(시술|치료|수술|성형|이식|교정)[^.!?\n]{0,6}(받으세요|받아보세요|받으[시실신]|받으러|하세요|해보세요|시작하세요|진행하세요|진행합니다)/,
   ...
   */
  {
    ...
    //
    // ⚠️ 2026-08-13 실측 오탐: *"통증이 조절되지 않으면 수술을 고려합니다"* 가 CRITICAL 로
    //    잡혔다. **조건절이지 단정이 아니다** — 오히려 치료 판단 기준을 설명하는 정상 문장이다.
    //    위 무위해 단정 규칙이 `없으면`·`없어도` 를 이미 배제하는 것과 같은 이유로,
    //    `않으면`·`않을 때`·`않는다면` 조건 어미를 배제한다. 단정 종결(않습니다·않아요·
    //    않는다)은 그대로 잡힌다. 조건 어미 3종만 좁게 배제해 회피구를 만들지 않는다.
    // ⚠️명사 목록을 HARM_NOUNS 로 통일한다. 5라운드에서 `출혈` 이 여기만 빠져
    //   "출혈이 발생하지 않습니다" 가 새는 게 드러났다.
    pattern: new RegExp(
      `(?:(${HARM_NOUNS})[^.!?\\n]{0,8}(남지|생기지|나타나지|되지|발생하지)|(재발|염증|감염|출혈)하지)` +
        `\\s*(는|도)?\\s*않(?!${COND_ENDING})`,
    ),
  ...
  /**
   * 조건 어미(`않으면`·`않을 때`·`않는다면`)는 위에서 CRITICAL 에서 뺐다 —
   * "통증이 조절되지 않으면 수술을 고려합니다" 는 치료 판단 기준을 설명하는 정상 문장이다.
   *
   * ⚠️2026-08-13 교차검증: 그런데 **완전 배제하면 조건절 뒤에 광고 결론을 붙여 우회**할 수 있다.
   *   "부작용이 발생하지 않으면 지금 시술받으세요", "부작용이 나타나지 않는다면 안심하고 선택하세요".
   *   회피 비용이 너무 낮다. ⇒ `없이도` 와 **같은 3단 구조**로 맞춘다:
   *   조건형 자체는 MEDIUM(게이트 미발동), 뒤에 CTA 가 붙으면 CRITICAL 승격.
   */
  {
    pattern: new RegExp(
      `(?:(${HARM_NOUNS})[^.!?\\n]{0,8}(남지|생기지|나타나지|되지|발생하지)|(재발|염증|감염|출혈)하지)` +
    ...
    message:
      '“…지 않으면”은 치료 판단 기준을 설명하는 정상 서술일 수 있지만, 뒤에 시술 권유가 붙으면 결과 보장 광고가 됩니다.',
    severity: 'MEDIUM',
    rule: '치료 결과 보장 주의 (의료법 제56조 제1항)',
    suggestion: '설명 문장이면 그대로 두어도 되지만, 권유로 이어지면 조건절을 삭제',
  },
  {
    // ⚠️MEDIUM 규칙과 **분기 수를 맞춘다.** `(재발|염증|감염|출혈)하지 않으면` 분기가
    //   승격에만 빠져 있어 "재발하지 않으면 지금 치료받으세요" 가 MEDIUM 에 머물렀다(3라운드 지적).
    pattern: new RegExp(
      `(?:(${HARM_NOUNS})[^.!?\\n]{0,8}(남지|생기지|나타나지|되지|발생하지)|(재발|염증|감염|출혈)하지)` +
        `\\s*(는|도)?\\s*않${COND_ENDING}[^.!?\\n]{0,40}(${AD_CTA})`,
    ),
    message: '조건절 뒤에 시술 권유가 이어지는 문장은 치료 결과 보장 광고입니다(의료법 제56조 제1항).',
    severity: 'CRITICAL',
    rule: '허위 안전성·치료 결과 보장 금지 (의료법 제56조 제1항)',
    suggestion: '조건절 + 권유 구조 삭제 — 발생 가능성과 판단 기준 안내로 대체',
  },
   ...
   */
  {
    // ⚠️2026-08-13: **부정 극성**을 배제한다. "치료 효과를 보장하지 않는다는 점을
    //   설명합니다" 는 오히려 의료광고법이 요구하는 고지 문구인데 CRITICAL 로 막고 있었다.
    //   `보장성`(보험 용어) 배제와 같은 계열의 좁은 예외다.
    // ⚠️조사에 `가|이` 가 빠져 있어 "효과**가** 보장되지 않을 리 없습니다" 가 통째로
    //   빠져나갔다(2026-08-13 4라운드에서 발견한 기존 구멍).
    pattern: new RegExp(
      `(효과|결과|만족|성과|효험|개선|호전|완화|성공)\\s*(를|을|은|는|가|이)?\\s*보장${NEG_NOT_DOUBLE}`,
    ),
...
export const MEDICAL_DEVICE_NAMES: readonly string[] = [
  ...
  '리쥬란', '쥬베룩', '스컬트라',
  // 2026-W32 주간 리서치 반영 (대표 승인 완료 2026-08-03) — 리프팅 하이엔드 장비.
  // `엠페이스`는 기존 `튠페이스`와 접미사만 겹칠 뿐 부분문자열 관계가 아니다(각각 독립 매칭).
  // ⚠️ `온다리프팅`은 **복합어로만** 등록한다 — 맨 `온다`는 동사 '오다'의 활용형
  //    ("환자가 온다")과 완전히 겹쳐서 등록하면 정상 글이 대량 오검출된다.
  // ⚠️ `울쎄라피 프라임`은 기존 `울쎄라` 부분매칭으로 이미 잡히므로 등록하지 않는다.
  // ⚠️ `하이포커스샷`은 해외 출시분이라 국내 도입 확인 후로 보류.
  '엠페이스', '엑시온', '온다리프팅',
];
...
/**
 * 생성 단계 경계선 — 1인칭 임상 관찰을 지시하는 프롬프트에 함께 붙인다.
 *
 * 왜 A층(checkCompliance)으로 못 막는가: A층은 어휘·패턴 필터라 "작년에 200건 수술했습니다"
 * 같은 문장을 구조적으로 잡을 수 없다(사실이면 합법이므로 잡으면 안 된다). 위 69번 줄 주석대로
 * "진료실에서 보면"류 일반화된 임상 관찰도 의도적으로 매칭하지 않는다.
 * 즉 **모델이 알 수 없는 병원 고유 사실을 지어내는 것**은 생성 시점에서만 막을 수 있다.
 *
 * ⚠️네이버·구글 프롬프트가 공유한다. 한쪽만 고치면 두 채널의 기준이 갈라지므로 여기서만 수정할 것.
 */
export const CLINICAL_OBSERVATION_LIMITS = `【1인칭 관찰의 한계선 — 병원 고유 사실은 지어내지 않는다】
관찰은 특정 병원·의사·환자의 실제 경험을 꾸며내지 말고, 의학적으로 널리 알려진
일반적 경향·전형적 진료 상황의 범위에서 쓴다. 개인차나 예외가 있는 내용은 경향으로
표현하고 조건을 함께 밝힌다. ("진료실에서 보면 ~한 분들이 많습니다" 같은 일반적 경향
서술은 그대로 써도 된다 — 금지 대상이 아니다.)

아래는 주어진 정보만으로는 사실 확인이 불가능하거나 의료광고에서 별도로 제한되는 것이라,
화자·문장 형식과 무관하게 사실처럼 보여도 임의로 생성하지 않는다:
- 실적·경력 — 시술·수술 건수, 진료 연차, 순위, 우수성 주장
- 자격·소속·수상·인증·지정기관·특허·논문·방송 출연
- 보유 장비, 최신·독점 장비 주장, 병원 고유 치료법·독자 프로토콜
- 병원 규모, 의료진 수, 누적 환자 수, 개원 연도, 가격·이벤트·보험 적용 여부
- 개인을 특정하거나 한 환자의 진료 과정을 재구성한 사례·서사·후기·전후 비교
- 근거 없는 최상급 빈도 — "가장 많이", "대부분", "거의 모두"
- 모든 환자에게 같은 진료 순서가 적용된다는 단정
  (진단·중증도·응급성·금기·환자 상태에 따라 달라진다는 점을 함께 밝힌다)`;
...
5. 검수기에 걸리는 "표기"만 피한다 — 설명 내용은 그대로 살릴 것:
   위해 명사(부작용·통증·흉터·붓기·염증·감염·재발·후유증·합병증) 바로 뒤에
   "없이 / 없는 / 없다 / 없습니다 / 되지 않 / 남지 않 / 생기지 않" 을 붙이지 말 것.
   무위해 단정 광고와 표기가 같아서, 뜻이 정반대인 경고 문장까지 자동 검수에 차단된다.
   ⚠️설명을 빼라는 뜻이 아니다. 같은 내용을 다른 표기로 쓴다:
   ❌ 통증 없이도 변형이 진행될 수 있습니다
   ✅ 아프지 않아도 변형이 진행될 수 있습니다 / 뚜렷한 증상이 나타나기 전에도 진행될 수 있습니다
   ❌ 통증이 조절되지 않으면 수술을 고려합니다
   ✅ 통증이 계속되면 수술을 고려합니다 / 약물과 보조기로 조절되지 않을 때 수술을 고려합니다
   ❌ 통증이 없는 초기 단계에서도 검사가 필요합니다
   ✅ 아직 아프지 않은 초기 단계에서도 검사가 필요합니다
   (단 "~없는 경우 / ~없는 환자 / ~없는 때 / ~없는 분" 형태는 그대로 써도 된다)
`.trim();
