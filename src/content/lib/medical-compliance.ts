...
const WARNING_PATTERNS: PatternRule[] = [
   ...
   *
   * ⚠️ 5라운드: "제외어를 끼워 넣는 회피는 어색해서 무시해도 된다"는 판단이 틀렸다.
   *    "부작용 걱정 없이 상담 후 시술받으세요", "흉터 걱정 없이 방문 상담 후
   *    수술받으세요" 는 광고에서 아주 자연스러운 연결이다.
   *    → 제외 조건에 **뒤에 시술 권유가 이어지지 않을 것**을 덧붙인다.
   *
   * ⚠️ 6라운드: 그 조건을 "뒤 어딘가에 시술 어휘가 있으면" 으로 구현한 것이 틀렸다.
   *    ① 오탐 — "걱정 없이 **상담받으**세요" 의 `받으` 를 시술 권유로 오인했다.
   *       제외어 자신의 활용형이 조건을 깨뜨렸다.
   *    ② 비용 — `[^.!?\n]*` 무제한 후방 탐색이라 후보마다 문장 끝까지 다시 훑어
   *       O(n²) 가 됐다. 앞 라운드에서 없앴다고 보고한 그 위험을 내가 되살렸다.
   * ★ 7라운드에서 **접근을 바꿨다.** 3~6라운드 내내 같은 일이 반복됐다 —
   *   예외를 좁히면 정상 문장이 막히고, 넓히면 광고가 빠져나갔다.
   *   ('후' 를 필수 신호로 본 6라운드 가정도 "상담하고 시술받으세요",
   *    "상담 및 시술 진행" 앞에서 무너졌다.)
   *   어떤 어휘 조합으로도 "환자의 걱정을 덜어주는 안내"와 "무위해 광고"를
   *   가를 수 없다. 그건 문맥 판단이고 A층의 일이 아니다.
   *
   *   → 예외를 **전부 걷어낸다**. 대신 두 갈래로 나눈다.
   *
   * ★ 8라운드에서 다시 고쳤다. 7라운드는 "거리로 확신도를 나눈다"였는데 두 가지가 틀렸다:
   *   ① 거리가 멀어도 명백한 광고가 있다 — "부작용에 대해 안심하세요, 걱정 없이
   *      시술받으세요" 가 MEDIUM 이라 발행 게이트를 그냥 통과했다.
   *   ② "제외어가 없으니 회피구가 없다"는 내 주장이 틀렸다. **창 자체가 회피구**다.
   *      20자를 넘기면 표면화조차 안 됐다 — 안내 문구를 길게 끼우기만 하면 됐다.
   *
   *   → 핵심을 잘못 쓰고 있었다. **뒤에 오는 시술 권유는 '예외 조건'이 아니라
   *     '승격 신호'** 다. 예외로 쓰면 회피구가 되고, 승격으로 쓰면 회피구가 안 된다.
   *     · 기본(아래 첫 규칙): 위해 명사 + 걱정 없이 → MEDIUM 표면화. 창을 45자로
   *       넓혀 긴 수식어 끼우기를 막는다.
   *     · 승격(아래 둘째 규칙): 그 뒤에 **시술·치료·수술 권유**가 이어지면 CRITICAL.
   *       '상담·문의' 는 시술 명사가 아니라 자동으로 빠진다(따로 예외를 두지 않는다).
   */
  {
    pattern: /(부작용|후유증|합병증|통증|흉터|재발)[^.!?\n]{0,45}(걱정|우려)\s*(도|은|는)?\s*없이/,
    message:
      '“걱정 없이” 표현은 위해가 없다는 인상을 줄 수 있어 심의 대상입니다. 안내 문맥이면 그대로 두어도 되지만, 시술을 권하는 문장이면 삭제가 필요합니다.',
    severity: 'MEDIUM',
    rule: '허위 안전성 인상 유발 표현 — 문맥 확인 필요 (의료법 제56조 제1항)',
    suggestion: '시술 권유 문장이면 “걱정 없이” 삭제 — 발생 가능성 안내로 대체',
  },
  // 승격 — "걱정 없이" 뒤에 시술·치료 권유가 붙으면 명백한 무위해 광고다.
  // 예외가 아니라 **추가 규칙**이라 제외어를 끼워 넣는 회피가 성립하지 않는다.
  {
    pattern:
      /(부작용|후유증|합병증|통증|흉터|재발)[^.!?\n]{0,45}(걱정|우려)\s*(도|은|는)?\s*없이[^.!?\n]{0,25}(시술|치료|수술|성형|이식|교정)/,
    message: '“걱정 없이” 뒤에 시술·치료 권유가 이어지는 문장은 무위해 단정 광고입니다.',
    severity: 'CRITICAL',
    rule: '허위 안전성·무위해 단정 금지 (의료법 제56조 제1항)',
   ...
   *      ② `치료` 를 조사 유무로 가르려던 시도는 **철회했다**(5라운드).
   *
   * ⚠️ 철회 근거: "보험 어휘는 조사가 없다"는 가정이 틀렸다. 보험 안내에서도
   *    "보험은 암 치료를 보장하는 상품입니다", "해당 특약은 입원 치료를 보장합니다",
   *    "질병 치료를 보장받을 수 있습니다" 처럼 **조사가 붙은 서술형**이 자연스럽다.
   *    이걸 CRITICAL 로 막으면 실손·보험 안내 글이 발행되지 않는다.
   *    → `치료를 보장` 단독 갈래를 뺐다. **성과 명사가 있어야 위반으로 본다.**
   *      "치료 효과/결과/성과를 보장" 은 그대로 잡힌다.
   *
   * ⚠️ 남는 공백(의도적): 성과 명사 없는 "치료를 보장합니다" 단독. 보험 문맥에서
   *    같은 표현이 정상적으로 쓰여 A층 어휘만으로는 가를 수 없다. 문맥 판단이
   *    필요한 층위이므로 B층 LLM 심의에 맡긴다. 여기서 잡으려다 정상 보험 안내를
   *    막는 쪽이 더 큰 손해다.
   */
  {
    pattern: /(효과|결과|만족|성과|효험|개선|호전|완화|성공)\s*(를|을|은|는)?\s*보장(?!성)/,
