...
const WARNING_PATTERNS: PatternRule[] = [
   ...
   *    → `없이` **뒤에 시술·치료를 받는 서술**이 오는 경우로 한정한다.
   *      '문의·상담·결정' 으로 이어지는 안내 문장은 빠진다.
   */
  {
    pattern:
      /(부작용|후유증|합병증|통증|흉터|재발)[^.!?\n]{0,20}(걱정|우려)\s*(도|은|는)?\s*없이[^.!?\n]{0,10}(받으|받을|받고|시술|치료|진행|이용)/,
  ...
  {
    pattern:
      /(부작용|후유증|흉터|붓기|통증|재발|염증)[^.!?\n]{0,8}(남지|생기지|나타나지|되지)\s*(는|도)?\s*않|(재발|염증|감염|출혈)하지\s*(는|도)?\s*않/,
   ...
   *      · `효과·결과·만족` + 보장 = 치료 성과 보장 → 문맥과 무관하게 언제나 위반
   *      · `치료 보장` 단독은 **보험 어휘**다(치료 보장 범위·한도·보장성 보험).
   *        그래서 머리 명사에서 뺐다. 대신 "치료 효과/결과를 보장" 은 위 갈래가 잡는다.
   */
  {
    pattern: /(효과|결과|만족)\s*(를|을|은|는)?\s*보장(?!성)/,
