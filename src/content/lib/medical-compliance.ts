...
const WARNING_PATTERNS: PatternRule[] = [
  ...
  // 간격 8자를 허용해 "부작용 걱정 없이"·"부작용이 전혀 없습니다" 를 함께 잡는다.
  {
     ...
     */
    pattern:
      /(부작용|후유증|합병증|통증|흉터|붓기|염증|감염|재발)[^.!?\n]{0,8}(?<!(?:안내|설명|고지|동의|상담)\s{0,2})(없이|없는(?!지|\s*(경우|환자|때|분|사람|케이스))|없다|없습니다|없어요|없음|없을(?!까))/,
  ...
  /**
   * "걱정 없이"·"우려 없이" — 안전 절차 서술과 절대 겹치지 않는 광고 문구다.
   * 위 패턴의 8자 창을 넘는 형태("부작용 설명 없이도 걱정 없이 …")까지 잡기 위해
   * 창을 넓게 잡은 전용 규칙을 둔다. 2라운드 교차검증 회피 사례 대응.
   */
  {
  ...
  // 보조사 결합("남지는 않습니다"·"남지도 않습니다")까지 받는다 — 2026-07-28 교차검증 지적.
  {
    pattern: /(부작용|후유증|흉터|붓기|통증|재발)[^.!?\n]{0,8}(남지|생기지|나타나지|하지|되지)\s*(는|도)?\s*않/,
   ...
   *
   * ⚠️⚠️ 2라운드 교차검증: 뒤따르는 명사(범위·내용·한도·기간·금액·대상)를 무조건
   *    배제했더니 **실제 위반이 통과**했다 — "효과 보장 기간은 평생입니다",
   *    "만족 보장 금액을 전액 환불해 드립니다", "결과 보장 대상은 모든 환자입니다".
   *    전부 자연스러운 광고 문구다. 명사만 보고 배제하면 회피구가 된다.
   *    → 배제 조건을 **같은 문장의 보험 문맥**으로 바꾼다. 보험 얘기를 하는 문장에서만
   *      빠지고, 보험 언급 없는 보장 주장은 전부 잡는다.
   */
  {
    pattern:
      /(효과|결과|만족|치료)\s*(를|을|은|는)?\s*보장(?!성)(?![^.!?\n]*(보험|보험사|가입|약관|급여))/,
