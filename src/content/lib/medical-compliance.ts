...
const WARNING_PATTERNS: PatternRule[] = [
  ...
  // 간격 8자를 허용해 "부작용 걱정 없이"·"부작용이 전혀 없습니다" 를 함께 잡는다.
  {
     ...
     *
     * ⚠️ 2026-07-28 교차검증에서 나온 오탐 2종을 추가로 배제한다:
     *    ① `없는 경우/환자/때/분` — 환자군·조건 설명이지 무위해 단정이 아니다.
     *       "통증이 없는 경우에도 검사가 필요합니다" 는 오히려 권장 문장이다.
     *    ② 간격에 `안내·설명·고지·동의·상담` 이 낀 형태 — **안전 절차 서술**이다.
     *       "부작용 안내 없이 시술하지 않습니다" 를 막으면 좋은 글이 발행되지 않는다.
     *       그래서 간격을 tempered 로 바꿔 해당 어휘가 들어오면 매칭을 포기한다.
     */
    pattern:
      /(부작용|후유증|합병증|통증|흉터|붓기|염증|감염|재발)(?:(?!안내|설명|고지|동의|상담)[^.!?\n]){0,8}(없이|없는(?!지|\s*(경우|환자|때|분|사람|케이스))|없다|없습니다|없어요|없음|없을(?!까))/,
   ...
   *
   * ⚠️ `보장성`·`보장 범위`·`보장 내용`·`보장 한도`는 **보험 안내 어휘**다.
   *    "치료 보장성 보험의 가입 조건을 확인하세요" 같은 정보성 문장을 CRITICAL 로
   *    막으면 안 된다(2026-07-28 교차검증 지적). 뒤따르는 보험 명사를 배제한다.
   */
  {
    pattern: /(효과|결과|만족|치료)\s*(를|을|은|는)?\s*보장(?!성)(?!\s*(범위|내용|한도|기간|금액|대상))/,
