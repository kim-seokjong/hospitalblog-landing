...
const WARNING_PATTERNS: PatternRule[] = [
   ...
   *
   *   → 예외를 **전부 걷어낸다**. 대신 **거리로 확신도를 나눈다**:
   *     · 위해 명사에 바로 붙은 "부작용 걱정 없이"(8자 창) → 위 주 패턴이 CRITICAL.
   *       이 어구 자체가 무위해 단정이라 뒤에 뭐가 오든 광고다.
   *     · 멀리 떨어진 형태(여기, 20자 창) → **MEDIUM 으로 표면화만** 한다.
   *       발행 게이트는 HIGH/CRITICAL 이므로 정상 안내를 막지 않으면서,
   *       검수 화면에는 반드시 뜬다.
   *
   *   이렇게 하면 회피구가 원리적으로 사라진다(제외어가 없으므로). 리콜은 100%,
   *   게이트 오판만 피한다. 확정 판단은 B층 LLM 심의 몫이다.
   */
  {
    pattern: /(부작용|후유증|합병증|통증|흉터|재발)[^.!?\n]{0,20}(걱정|우려)\s*(도|은|는)?\s*없이/,
    message:
