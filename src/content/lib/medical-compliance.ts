...
const WARNING_PATTERNS: PatternRule[] = [
  ...
  // 2026-W30: '전문병원' 명칭 무단 표방 — 복지부 지정 없이 사용 금지(의료법 제3조의5 제4항).
  //
  // 범위를 '전문병원' 으로만 좁힌다. 조문이 규제하는 명칭이 '전문병원' 이고,
  // '전문 의원·클리닉·센터' 까지 HIGH 로 묶으면 일반 표현이 발행 게이트를 차단한다.
  // 오탐 배제 2종:
  //  - "전문의와 상담"(권장 마무리 문구)은 병원 명사 결합이 아니라 매칭되지 않는다.
  //  - 앞쪽 20자 안에 '지정'이 있으면 제외 — "보건복지부 지정 척추 전문병원"처럼
  //    지정 사실을 적법하게 기술한 경우(지정 표현은 명칭 **앞**에 온다).
  {
    pattern: /(?<!지정[^.!?\n]{0,20})전문\s*병원/,
    message: '‘전문병원’ 명칭은 보건복지부 지정 기관만 사용할 수 있습니다(의료법 제3조의5).',
    severity: 'HIGH',
    rule: '전문병원 명칭 무단 표방 금지 (의료법 제3조의5 제4항)',
