...
const MUST_CATCH: ReadonlyArray<readonly [string, string]> = [
  ...
  ['재발 없이', '재발 없이 유지됩니다.'],
  /**
   * `없이도` 를 양보형으로 배제하면서 생기는 구멍을 **승격**으로 막는다.
   * 예외는 배제어를 끼워 넣어 회피할 수 있지만, 승격은 추가 규칙이라 회피가 성립하지 않는다
   * (2026-07-28 9라운드 교차검증의 결론). "없이도" 뒤에 시술·치료 **권유**가 붙으면
   * 양보 서술이 아니라 무위해 광고다.
   */
  ['통증 없이도 + 시술 권유', '통증 없이도 편하게 시술받으세요.'],
  ['부작용 없이도 + 치료 권유', '부작용 없이도 안전하게 치료받으실 수 있습니다.'],
  ['흉터 없이도 + 수술 권유', '흉터 없이도 만족스러운 수술이 가능합니다.'],
  /**
   * 2026-08-13 교차검증에서 나온 **회피 사례**. 처음엔 `없이(?!도)` 로 완전 배제했다가
   * 아래가 전부 통과했다. 시술 명사 없이 결과·CTA 만 있는 형태가 특히 잘 샜다.
   * 이제는 기본 MEDIUM + 승격 구조라 최소한 탐지는 되고, 아래는 CRITICAL 로 올라와야 한다.
   */
  ['없이도 + 결과 CTA', '부작용 없이도 만족스러운 결과를 경험하세요.'],
  ['없이도 + 만나보세요', '통증 없이도 아름다운 변화를 만나 보세요.'],
  ['없이도 + 누려보세요', '흉터 없이도 자연스러운 개선 효과를 누려 보세요.'],
  ['없이도 + 받을 수', '부작용 없이도 치료받을 수 있습니다.'],
  ['없이도 + 받아 보실', '부작용 없이도 치료를 받아 보실 수 있습니다.'],
  ['없이도 + 해드립니다', '통증 없이도 안전하게 시술해 드립니다.'],
  ['없이도 + 임플란트', '통증 없이도 임플란트를 받을 수 있습니다.'],
  ['없이도 + 주사', '부작용 없이도 주사를 받아 보세요.'],
  ['없이도 + 레이저', '흉터 없이도 레이저 관리를 받아 보세요.'],
  ['없이도 + 검사', '통증 없이도 검사를 받으실 수 있습니다.'],
  ['없이도 + 안심하고 선택', '부작용 없이도 안심하고 선택하실 수 있습니다.'],
  /** 2라운드 교차검증 — 위해 명사 목록에 없던 것들(출혈·멍)과 조건절 뒤 광고 결론. */
  ['출혈 없이도 + 수술 권유', '출혈 없이도 안전하게 수술받으세요.'],
  ['조건절 + 시술 권유', '부작용이 발생하지 않으면 지금 시술받으세요.'],
  ['조건절 + 치료 시작', '통증이 생기지 않을 때 바로 치료를 시작하세요.'],
  ['조건절 + 안심하고 선택', '부작용이 나타나지 않는다면 안심하고 선택하세요.'],
  /** 3라운드 — 승격에 빠져 있던 `(재발|염증|감염|출혈)하지 않으면` 분기. */
  ['재발하지 않으면 + 치료 권유', '재발하지 않으면 지금 치료받으세요.'],
  ['출혈하지 않으면 + 시술 권유', '출혈하지 않으면 바로 시술받으세요.'],
  /** 3라운드 — 기본 `없이` 규칙에 출혈·멍이 빠져 탐지 0건이던 것. */
  ['출혈 없이 + 수술 권유', '출혈 없이 안전하게 수술받으세요.'],
  /**
   * ⛔의도적 공백: "멍 없이 자연스럽게 시술받으세요" 는 **A층에서 잡지 않는다.**
   * `멍` 은 한 글자라 경계를 아무리 둘러도 합성어 오탐이 계속 나온다(구멍·멍울·멍게·멍에…).
   * 정상 글을 막는 손해가 더 커서 위해 명사에서 뺐다 — 문맥 판단은 B층 LLM 심의의 몫이다.
   * 되돌리려면 위 MUST_NOT_CATCH 의 `멍*` 4건이 함께 통과하는지부터 확인할 것.
   */
  /**
   * 2026-08-13 발견한 기존 리콜 구멍. 승격 규칙의 후행 동사 허용목록이 `받으시` 였는데
   * "받으실"의 마지막 음절은 `시`가 아니라 **`실`** 이라 매칭되지 않았다.
   * "…걱정 없이 치료받으실 수 있습니다" 가 통째로 빠져나가고 있었다. `받으[시실신]` 으로 넓혔다.
   */
  ['걱정 없이 + 받으실', '부작용 걱정 없이 치료받으실 수 있습니다.'],
  ['걱정 없이 + 받으신', '통증 걱정 없이 시술받으신 분들이 많습니다.'],
  /** 조건 어미를 배제하면서 단정 종결이 새지 않는지 고정한다(위 오탐 수정의 짝). */
  ['흉터가 남지 않습니다', '한 번 시술로 흉터가 남지 않습니다.'],
  ['통증이 생기지 않아요', '시술 후 통증이 생기지 않아요.'],
  ['부작용이 발생하지 않는다', '이 시술은 부작용이 발생하지 않는다.'],
  ['부작용이 나타나지 않습니다', '부작용이 나타나지 않습니다.'],
  ['재발하지 않습니다', '한 번 시술로 재발하지 않습니다.'],
  ['효과를 보장', '효과를 보장해 드립니다.'],
  /** 부정 극성 배제가 긍정 단정까지 풀어버리지 않는지 고정한다(2026-08-13). */
  ['결과를 보장합니다', '수술 결과를 보장합니다.'],
  ['만족을 보장', '만족을 보장해 드리겠습니다.'],
  /**
   * 4라운드 — 부정 극성 배제가 **이중부정 광고**까지 면제하면 안 된다.
   * "보장하지 않는 것은 아니다" = 의미상 결과 보장이다.
   */
  ['이중부정: 않는 것은 아니', '결과를 보장하지 않는 것은 아닙니다.'],
  ['이중부정: 않을 수 없', '결과를 보장하지 않을 수 없습니다.'],
  ['이중부정: 않을 리 없', '효과가 보장되지 않을 리 없습니다.'],
  ['이중부정: 않는 게 아니', '결과를 보장하지 않는 게 아닙니다.'],
  ['이중부정: 않는 건 아니', '결과를 보장하지 않는 건 아닙니다.'],
  ['이중부정: 않는다고 볼 수 없', '결과를 보장하지 않는다고 볼 수 없습니다.'],
  ['이중부정: 않을 수는 없', '결과를 보장하지 않을 수는 없습니다.'],
  ['이중부정: 않을 수밖에 없', '결과를 보장하지 않을 수밖에 없습니다.'],
  /** 6라운드 회귀 — 조사 `는` 이 빠져 이중부정을 놓치던 것. */
  ['이중부정: 않는 바는 아니', '결과를 보장하지 않는 바는 아닙니다.'],
  ['이중부정: 할 수는 없', '결과를 보장하지 않는다고 할 수는 없습니다.'],
  /** 5라운드 — 명사 목록을 HARM_NOUNS 로 통일하면서 `출혈` 이 새던 것. */
  ['출혈이 발생하지 않습니다', '출혈이 발생하지 않습니다.'],
  ['출혈이 발생하지 않으면 + 권유', '출혈이 발생하지 않으면 지금 시술받으세요.'],
  /** 5라운드 — `받으셔`·`받으셨` 활용형 누락. */
  ['받으셔 보세요', '부작용 없이도 치료받으셔 보세요.'],
  ['받으셨으면', '통증 없이도 시술받으셨으면 합니다.'],
  ['확실히 개선', '확실히 개선됩니다.'],
  ['반드시 좋아집니다', '반드시 좋아집니다.'],
...
const MUST_NOT_CATCH: ReadonlyArray<readonly [string, string]> = [
  ...
  /**
   * 2026-08-13 실측 오탐. 생성된 글에서 아래 문장이 CRITICAL 로 잡혀 발행 게이트가 걸렸다.
   * 무위해 광고가 아니라 **무증상 진행을 경고**하는 문장이다 — 뜻이 정반대다.
   *
   * 원인: `없어도`·`없으면`·`없는지` 는 조건·양보 서술이라 이미 배제돼 있는데,
   * 같은 계열인 **`없이도` 만 빠져 있었다**(`없이` 로 매칭). 예외를 넓힌 게 아니라
   * 원래 있던 양보형 목록을 완성한 것이다. 광고로 쓰려면 `도` 가 붙을 이유가 없다
   * — "부작용 없이 안전하게"는 광고지만 "부작용 없이도"는 광고 문장이 되지 않는다.
   * 그래도 남는 구멍은 아래 MUST_CATCH 의 승격 규칙이 막는다.
   */
  /**
   * 2026-08-13 실측 오탐 2. "조절되지 않으면" 은 **조건절**이지 단정이 아니다.
   * 치료 판단 기준을 설명하는 정상 문장인데 CRITICAL 로 발행이 막혔다.
   * 단정 종결(않습니다·않아요)은 아래 MUST_CATCH 가 그대로 고정한다.
   */
  /**
   * ⚠️조건절(`않으면`·`않을 때`·`않는다면`)은 여기(위반 0건)가 아니라 아래
   *   CONCESSIVE_MUST_NOT_BLOCK 에서 검증한다 — 2라운드 교차검증에서 **완전 배제하면
   *   조건절 뒤에 광고 결론을 붙여 우회**할 수 있는 게 드러나, MEDIUM 표면화 + CTA 승격으로
   *   바꿨기 때문이다. 발행을 막지 않는다는 점이 검증 대상이지 검출 0건이 아니다.
   */
  ['호전되지 않을 때', '증상이 호전되지 않을 때 추가 검사를 진행합니다.'],
];
...
/* ════════════════════════════════════════════════════════════════════════
 * `없이도` 3단 구조 (2026-08-13)
 *
 * 정상 경고 서술은 **발행을 막지 않아야** 하고(MEDIUM 표면화까지는 허용),
 * 광고는 CRITICAL 로 올라와야 한다. 완전 배제하지 않는 이유 = 허용목록에 구멍이
 * 생겨도 최악이 MEDIUM 이라 **탐지 자체가 0이 되지 않기** 때문이다.
 * ════════════════════════════════════════════════════════════════════════ */
const blocksPublishing = (r: ReturnType<typeof checkCompliance>) =>
  r.violations.some((v) => v.severity === 'CRITICAL' || v.severity === 'HIGH');

const CONCESSIVE_MUST_NOT_BLOCK: ReadonlyArray<readonly [string, string]> = [
  ['통증 없이도(진행 경고)', '통증 없이도 변형이 진행될 수 있어 초기 증상을 놓치기 쉽습니다.'],
  ['염증 없이도', '염증 없이도 관절 손상이 진행될 수 있습니다.'],
  ['흉터 없이도', '흉터 없이도 색소 침착이 남을 수 있습니다.'],
  ['재발 없이도', '재발 없이도 정기적인 추적 관찰이 필요합니다.'],
  ['통증 없이도 + 조기 발견', '통증 없이도 조기 발견이 가능합니다.'],
  ['부작용 없이도 + 안내', '부작용 없이도 정기 점검이 필요하다는 점을 안내드립니다.'],
  /** 2026-08-13 2라운드 교차검증 — 승격 규칙의 짧은 조각이 물던 정상 문장들. */
  ['안심하고 방치 금지(경고)', '통증 없이도 병이 진행될 수 있으니 안심하고 방치해서는 안 됩니다.'],
  ['사회보장 안내', '통증 없이도 질환이 진행될 수 있어 사회보장 제도를 함께 안내합니다.'],
  ['받아들이기', '부작용 없이도 치료 정보를 받아들이기 쉽습니다.'],
  ['가능성 설명', '통증 없이도 치료 가능성을 설명합니다.'],
  ['제공되는 정보', '통증 없이도 치료에서 제공되는 정보를 안내합니다.'],
  /** 3라운드 교차검증 — AD_CTA 종결형이 정상 안내·부정문을 물던 사례. */
  ['정보를 제공합니다', '통증 없이도 질환이 진행될 수 있어 관련 정보를 제공합니다.'],
  ['안내해 드립니다', '통증 없이도 진행될 수 있음을 안내해 드립니다.'],
  ['치료를 안내해 드립니다', '통증 없이도 치료를 안내해 드립니다.'],
  ['보장하지 않는다', '통증 없이도 진행될 수 있어 치료 효과를 보장하지 않는다는 점을 설명합니다.'],
  ['선택해서는 안 됩니다', '통증 없이도 병이 진행되므로 안심하고 치료를 선택해서는 안 됩니다.'],
  /** `멍` 은 한 글자라 좌측 경계가 없으면 합성어를 문다. */
  ['구멍 없이', '보철물은 구멍 없이 정밀하게 제작됩니다.'],
  /**
   * `멍` 은 한 글자라 경계를 둘러도 합성어가 계속 나와 **위해 명사에서 아예 뺐다**(5라운드).
   * 아래가 전부 통과해야 한다 — 하나라도 걸리면 정상 글이 발행에서 막힌다.
   */
  ['멍울 없이', '멍울 없이 정밀하게 검사합니다.'],
  ['멍게 없이', '멍게 없이 담백하게 끓인 국을 권합니다.'],
  ['멍에 없이', '멍에 없이 회복 과정을 함께합니다.'],
  ['멍하니 없이', '멍하니 보내는 시간 없이 재활에 집중합니다.'],
  /** 조건·양보 어미는 정상 임상 서술이다(5라운드 추가분). */
  ['않을 경우', '증상이 호전되지 않을 경우 추가 검사를 진행합니다.'],
  ['않는 경우', '통증이 조절되지 않는 경우 수술을 고려합니다.'],
  ['않더라도', '증상이 나타나지 않더라도 추적 관찰이 필요합니다.'],
  ['않아도', '재발하지 않아도 정기 검진이 필요합니다.'],
  /** 부정 극성 배제(단순 부정)는 정상 고지문이므로 잡히면 안 된다. */
  ['보장하지 않는다', '치료 효과를 보장하지 않는다는 점을 미리 설명드립니다.'],
  ['보장할 수 없습니다', '개인차가 있어 결과를 보장할 수 없습니다.'],
  /** 조건절 자체는 발행을 막지 않는다(정상 치료 판단 서술). */
  ['조절되지 않으면', '통증이 조절되지 않으면 수술을 고려합니다.'],
  ['호전되지 않을 때', '증상이 호전되지 않을 때 추가 검사를 진행합니다.'],
  ['나타나지 않으면', '부작용이 나타나지 않으면 그대로 유지합니다.'],
  ['재발하지 않으면', '재발하지 않으면 추적 간격을 늘립니다.'],
];

for (const [label, text] of CONCESSIVE_MUST_NOT_BLOCK) {
  test(`checkCompliance(양보형): "${label}" 은 발행을 막지 않는다`, () => {
    const r = checkCompliance(text);
    assert.equal(
      blocksPublishing(r),
      false,
      `정상 서술이 발행 게이트에 걸렸다: « ${text} » → ${r.violations.map((v) => `${v.severity}:${v.word}`).join(', ')}`,
    );
  });
}

test('checkCompliance(양보형): 정상 서술이어도 탐지 자체는 남는다(MEDIUM 표면화)', () => {
  const r = checkCompliance('통증 없이도 변형이 진행될 수 있습니다.');
  assert.ok(
    r.violations.length > 0 || r.warnings.length > 0,
    '“없이도”는 등급을 낮추더라도 탐지는 유지되어야 한다 — 리콜이 0이 되면 안 된다',
  );
});
