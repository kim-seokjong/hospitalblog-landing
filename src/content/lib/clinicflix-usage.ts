...
/**
 * 변환 가능 여부 판정.
 *  - limits.video <= 0 (블로그 전용 플랜) → upgrade_required
 *  - video_used >= limits.video (무제한 -1 제외) → video_limit
 *  - channels: -1 이면 fairUseCap 을 소프트캡으로 사용, 아니면 limits.channels 하드캡
 */
export function checkQuota(planId: PlanId, usage: UsageRow): QuotaCheck {
  ...
  // 영상 한도 (-1 = 무제한)
  if (videoLimit !== -1 && usage.video_used >= videoLimit) {
    return {
      ok: false,
      reason: 'video_limit',
  ...
  // 채널 한도: -1 이면 fairUseCap(공정사용 소프트캡), 아니면 limits.channels 하드캡
