 ...
 *
 * ★ 개인정보 원칙 — 주장 범위를 정확히 한정한다:
 *   **이 기능의 테이블(clinic_ai_referrals)과 RPC 인자에는** 방문자 식별자가 없다.
 *   저장되는 것은 병원 slug · 출처 · 글 id · KST 일자, 그 넷뿐이고 타임스탬프도 없다.
 *
 *   범위 밖(우리가 없앨 수 없는 것): 이 비콘은 공개 HTTP API 를 거치므로
 *   **Vercel/CDN 액세스 로그에는 통상 요청 시각과 IP 가 남는다.** 요청 처리 중
 *   IP·UA 가 메모리에 스치는 것도 마찬가지다. 그래서 "수집하지 않는다"가 아니라
 *   "이 기능의 DB 에 저장하지 않는다"고 말한다.
...
// 서명 토큰
//
// ★ 이 토큰이 실제로 보증하는 것 (과대 표현 금지 — 2차 리뷰 지적 반영):
//   - 보증한다: 서명이 **우리 서버에서 발급**됐고(오프라인 위조 불가), 발급 후
//     10분 안이며, 적재되는 (slug·source·postId) 가 서명 시점과 동일하다.
//   - 보증하지 않는다: 요청자가 사람인지, 정말 AI 서비스에서 왔는지, 그 토큰이
//     처음 쓰이는지. 발급 경로도 병원 페이지도 공개라 **누구나 유효 토큰을 얻는다.**
//   따라서 이것은 "위조 방어"가 아니라 **위조 비용을 올리는 장치**다: 오프라인
//   대량 생성을 막고, 온라인 왕복을 강제하고, 재사용 창을 10분으로 좁힌다.
//   실제로 피해 규모를 묶는 것은 발신원 단위 레이트리밋이다.
// ---------------------------------------------------------------------------

/**
 * 토큰 수명. 비콘은 발급 직후(수 초 내) 발사되므로 짧아도 충분하고,
 * 짧을수록 유출된 토큰의 재사용 창이 좁아진다.
 *
 * ⚠️ 이 값을 늘려 "캐시된 페이지의 만료 문제"를 해결하려 하지 마라. 토큰은 방문
 *    시점에 별도 동적 경로(/api/clinic-site/ai-referral/token)에서 발급되므로
 *    페이지 캐시 수명과 무관하다. TTL 을 늘리면 재사용 창만 넓어진다.
 */
...
/**
 * 서명 대상 문자열 — **적재되는 값 전부**(병원·출처·글) + 만료시각.
 *
 * source 까지 넣는 이유: 서명 대상 밖의 필드가 있으면 "서명된 것"과 "적재되는 것"이
 * 달라진다. source 를 임의로 바꿔도 어차피 새 토큰을 받으면 그만이라 보안 강도가
 * 올라가지는 않지만, 비용이 0 이고 서명 범위 = 적재 범위라는 불변식을 지킬 수 있다.
 *
 * 구분자(`|`)는 slug(소문자 영숫자·하이픈)·source(소문자 영숫자·언더바)·uuid·숫자
 * 어디에도 나타나지 않아 필드 경계가 모호해지지 않는다.
 */
export function buildBeaconSigningInput(
  ...
  return `v2|${slug}|${source}|${postId ?? ''}|${exp}`;
}
