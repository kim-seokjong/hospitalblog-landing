  ...
  const handlePublish = async () => {
    ...
    try {
      const res = await fetch('/api/publish-naver', {
        ...
        body: JSON.stringify({ title, body: content.body, tags: tagList }),
      });
    ...
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
      ...
      <div className="px-5 py-4 space-y-3">
        ...
        <div className="bg-gray-50 rounded-xl p-3">
          <p className="text-xs font-semibold text-gray-500 mb-1">발행 예정 글</p>
        ...
        {status === 'publishing' && (
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 space-y-2">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-semibold text-blue-700">발행 중...</p>
            </div>
            <p className="text-xs text-blue-600">
              브라우저가 자동으로 열려 네이버 블로그에 글을 작성합니다.
              CAPTCHA가 표시되면 직접 해결해주세요.
            </p>
          </div>
        )}
