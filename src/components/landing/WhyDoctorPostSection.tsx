...
const REASONS = [
  ...
  },
  {
    icon: '🔭',
    title: '전 검색 표면 최적화',
    desc: '네이버·구글은 물론 AI 검색(GEO)까지 대응하고, 상위 노출 글의 구조를 역분석해 생성에 반영해요.',
  },
  {
    icon: '🎬',
      ...
      <div className="max-w-6xl mx-auto px-5 sm:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-10 sm:mt-12">
          {REASONS.map(({ icon, title, desc }) => (
            <div
              key={title}
              className="bg-white border border-[#dbe2ea] rounded-2xl p-6 sm:p-7 transition-transform hover:-translate-y-0.5"
            >
