...
//  ⛔② 상담 컷의 종이가 **백지**였다. 상담 장면인데 아무것도 안 적혀 있었다.
//     ⇒ 지금까지 "글자는 깨지니 넣지 마라"가 규칙이었는데 그건 `soul_2` 기준이다.
//       **`gpt_image_2` 는 한글을 제대로 그린다**(실측).
//       `nano_banana_pro` 는 같은 프롬프트에서 한글이 뭉개졌다 — 쓰지 말 것.
//
//  ★★★상담 컷은 **두 번 만에** 맞췄다. 1차는 종이를 크게 잡아 글자를 또렷하게 냈는데
//    대표가 *"문진표가 너무 잘 보이잖아. 어색해"* 라고 했다. **원본 구도가 이미 좋았고
//    비어 있던 종이만 문제였던 것**을 내가 「글자를 잘 보이게」로 잘못 읽었다.
//    ⇒ 2차는 **원본을 참조 이미지(`medias`)로 넣고 종이만 바꿨다.** 구도·인물·조명이 그대로 남는다.
//      이것이 오늘 릴스에서 배운 「구도 설계」의 실제 사용법이다 — 말로 재현하지 말고 원본을 준다.
//    ⚠️종이 내용은 **문진표가 아니라 진료기록부**로 갔다(대표: *"상담 내용이나 병원 차트도 괜찮다"*).
//      인쇄 표 + 손글씨가 섞이면 실제 차트처럼 읽히고, 글자가 작아도 어색하지 않다.
//      ⛔손글씨만 있는 안은 버렸다 — 한글 손글씨는 뭉개져서 가짜 티가 난다.
//      ⛔문진표 안은 「성영」 같은 오타가 났다. **작은 글자에 정자체를 요구하지 말 것.**
//  ★★★3차 — 대표: *"종이의 방향이 환자방향으로 놓아졌으면해. 지금 방향은 카메라방향이잖아"*
//    서류를 **카메라가 읽기 좋게** 놓아 두고 있었다. 의사가 환자에게 설명하는 중이면
//    종이는 **환자 쪽으로 돌아가 있어야** 한다 — 카메라에서 보면 글자가 옆으로 눕는다.
//    ⇒ ★**소품의 방향은 「카메라」가 아니라 「그 장면 안에서 그것을 읽는 사람」이 정한다.**
//      화면에 잘 보이게 두면 그 순간 연출 사진이 된다. 계절 어긋남과 같은 계열의 실수다.
//  화질도 함께 올렸다: 생성 2048×1360(2k/high) → 1350×900 으로 다운스케일(같은 규격, 더 선명).
//  원본은 `바탕 화면\닥포블로그이미지_20260906\원본백업\` 에 있다.
const BLOG_IMAGES = [
  { src: `${SAMPLE_BASE}/blog-image-1.jpg`, alt: '상담실에서 진료 기록을 함께 보며 설명을 듣는 장면', caption: '상담' },
